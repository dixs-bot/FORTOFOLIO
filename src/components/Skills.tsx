...
export default function Skills() {
  return (
    <section id="skills" className="py-24 border-t border-slate-900 bg-slate-950/50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {skillsData.map((category, catIdx) => (
            <motion.div
              ...
              className="p-8 bg-slate-900/20 border border-slate-800 rounded-2xl shadow-sm hover:border-slate-750 transition-colors"
            >
                    ...
                    <div key={skill.name}>
                      <div className="flex justify-between items-center mb-2.5">
                        <span className="text-sm font-semibold text-slate-200 flex items-center gap-2">
                          {SkillIcon && <SkillIcon size={16} className="text-slate-400" />}
                          {skill.name}
                        </span>
